...
public class NMEAParserTest {
	...
	public NMEAParserTest() throws ScriptException {
		...
		jsRoot = new File("./src/main/resources/");
		logger.debug("Javascript jsRoot: {}", jsRoot.getAbsolutePath());
		
		try {
			engine.eval(FileUtils.readFileToString(new File(jsRoot,"jsext/nashorn-polyfill.js")));
	...
	public void test() throws FileNotFoundException, ScriptException, NoSuchMethodException {
		try {
			...
			engine.eval(new java.io.FileReader(new File(jsRoot, "signalk-parser-nmea0183/dist/bundle.js")));
			...
			String hooks = FileUtils.readFileToString(new File(jsRoot,"signalk-parser-nmea0183/hooks-es5/supported.txt"));
			...
			/*
			 * RMC Sentence http://www.gpsinformation.org/dale/nmea.htm#RMC
			 * $GPRMC,123519,A,4807.038,N,01131.000,E,022.4,084.4,230394,003.1,W*6A values:
			 * - RMC Recommended Minimum sentence C [0] 123519 Fix taken at 12:35:19 UTC [1]
			 * A Status A=active or V=Void. [2][3] 4807.038,N Latitude 48 deg 07.038' N
			 * [4][5] 01131.000,E Longitude 11 deg 31.000' E [6] 022.4 Speed over the ground
			 * in knots [7] 084.4 Track angle in degrees True [8] 230394 Date - 23rd of
			 * March 1994 [9][10] 003.1,W Magnetic Variation - *6A The checksum data, always
			 * begins with *
			 */
			String rmc = new String("$GPRMC,123519,A,4807.038,N,01131.000,E,022.4,084.4,230394,003.1,W*6A");
			// String input = new String("{\"id\":\"RMC\",\"sentence\":\"xx\", \"parts\":
			// [123519,A,4807.038,N,01131.000,E,022.4,084.4,230394,003.1,W],\"tags\":{\"source\":\"xx\"}}");
			Object result = inv.invokeMethod(parser,"parse", rmc);
			...
			// String input = new String("{\"id\":\"RMC\",\"sentence\":\"xx\", \"parts\":
			// [123519,A,4807.038,N,01131.000,E,022.4,084.4,230394,003.1,W],\"tags\":{\"source\":\"xx\"}}");
			result = inv.invokeMethod(parser,"parse", rmc);
